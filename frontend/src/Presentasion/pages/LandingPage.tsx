...
import { Flame, BarChart2, Bell, Camera, Cookie, Lock, MapPin, Megaphone, Settings, X, Plus, Check, UserPlus, Target, CalendarCheck, TrendingUp } from 'lucide-react'
...
async function getLocationFromBrowser(): Promise<{ country?: string; city?: string }> {
  if (!navigator.geolocation) return {}
  try {
    const pos = await new Promise<GeolocationPosition>((resolve, reject) =>
      navigator.geolocation.getCurrentPosition(resolve, reject, {
        timeout: 10000,
        maximumAge: 0, // selalu minta posisi terbaru, tidak pakai cache
        enableHighAccuracy: false,
      })
    )
    const { latitude, longitude } = pos.coords
    // BigDataCloud: dirancang untuk browser, CORS-friendly, gratis tanpa API key
    ...
    return {} // user menolak izin atau timeout — tetap kirim tanpa lokasi
  }
}

async function sendTrackingHit(): Promise<void> {
  ...
  fetch('/api/track', {
    ...
    body: JSON.stringify({ page: 'landing', session_id: getOrCreateSessionId(), ...location }),
  }).catch(() => {})
}
    ...
    <div className="font-body bg-surface text-ink min-h-screen overflow-x-hidden">
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-body font-bold text-[18px] text-ink">HabitTracker</span>
        </div>
      ...
      >
        ...
        <div className="max-w-[1200px] mx-auto py-20 px-10 w-full flex items-center gap-16 flex-col md:flex-row">
          ...
          <div className="flex-1 animate-fade-in text-center md:text-left">
            ...
            <h1 className="font-body font-bold leading-[1.08] tracking-[-2px] mb-6 text-white" style={{ fontSize: 'clamp(40px, 6vw, 76px)' }}>
              Bangun Kebiasaan<br />
            ...
            <div className="mt-12 flex gap-8 flex-wrap">
              {[{ v: '4.9★', l: 'Rating Pengguna' }, { v: '100%', l: 'Gratis' }].map(s => (
                <div key={s.l}>
                  <div className="font-body text-[26px] font-bold text-white">{s.v}</div>
          ...
          <div className="hidden md:block flex-[0_0_420px] animate-float">
            <div className="rounded-[24px] p-7 bg-white/[0.07] backdrop-blur-xl border border-white/[0.12] shadow-[0_32px_80px_rgba(0,0,0,0.4)]">
              <div className="flex items-center justify-between mb-5">
                <span className="font-body font-bold text-white text-base">Habit Hari Ini</span>
              ...
              <div className="mt-5 flex gap-3">
                <div className="flex-1 rounded-[10px] py-[14px] px-3 text-center bg-white/5">
                  <div className="text-[22px] font-bold font-body text-accent-light">2/4</div>
                  <div className="text-[11px] mt-[3px] text-white/40">Selesai Hari Ini</div>
                </div>
                <div className="flex-1 rounded-[10px] py-[14px] px-3 text-center bg-white/5">
                  <div className="text-[22px] font-bold font-body flex items-center justify-center gap-1 text-[#ff6b35]">14 <Flame size={18} color="#ff6b35" /></div>
        ...
        <div className="max-w-[1100px] mx-auto grid grid-cols-2 md:grid-cols-3 gap-6">
          {stats.map((s, i) => (
            <div key={i} className="bg-white border border-border rounded-md p-7 text-center shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
              <div className="font-body text-[36px] font-bold text-primary">{s.value}</div>
      ...
      <section id="about" className="py-[100px] px-10 bg-surface">
        <div className="max-w-[1100px] mx-auto flex justify-center">
          ...
          >
            ...
            <h2 className="font-body font-bold mb-5 tracking-[-1px] leading-[1.15]" style={{ fontSize: 'clamp(28px, 4vw, 44px)' }}>
      ...
      <section id="services" className="py-[100px] px-10 bg-white">
        <div className="max-w-[1100px] mx-auto">
          <div className="text-center mb-16">
            <span className={sectionLabel}>Cara Penggunaan</span>
            <h2 className="font-body font-bold mb-4 tracking-[-1px]" style={{ fontSize: 'clamp(28px, 4vw, 46px)' }}>
              Mulai dalam 4 langkah mudah
            </h2>
            <p className="text-muted text-[17px] max-w-[520px] mx-auto leading-[1.7]">
              Tidak perlu konfigurasi rumit. Daftar sekarang dan bangun kebiasaan pertamamu dalam hitungan menit.
            </p>
          </div>

          {/* Step connector line (desktop only) */}
          <div className="relative">
            <div className="hidden lg:block absolute top-[52px] left-[calc(12.5%+28px)] right-[calc(12.5%+28px)] h-[2px] bg-gradient-to-r from-primary/20 via-primary/50 to-primary/20" />

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-5">
              {([
                {
                  step: 1,
                  icon: <UserPlus size={26} color="#16a34a" />,
                  title: 'Daftar Akun',
                  desc: 'Buat akun gratis dalam hitungan detik. Isi nama, email, dan kata sandi — tidak perlu kartu kredit atau verifikasi panjang.',
                  tag: 'Gratis selamanya',
                },
                {
                  step: 2,
                  icon: <Target size={26} color="#16a34a" />,
                  title: 'Buat Habit',
                  desc: 'Tambahkan habit yang ingin dibangun, tentukan periode target, dan aktifkan pengingat harian agar tidak terlewat.',
                  tag: 'Tanpa batas',
                },
                {
                  step: 3,
                  icon: <CalendarCheck size={26} color="#16a34a" />,
                  title: 'Centang Setiap Hari',
                  desc: 'Buka aplikasi, centang habit yang sudah selesai. Sistem otomatis mencatat streak dan progress secara real-time.',
                  tag: 'Streak otomatis',
                },
                {
                  step: 4,
                  ...
                  desc: 'Lihat laporan lengkap tiap habit selesai, pantau streak terpanjang, dan bagikan pencapaianmu ke komunitas.',
                  tag: 'Laporan & komunitas',
                },
              ] as const).map((s, i) => (
                <div
                  key={i}
                  id={`svc-${i}`}
                  data-observe
                  className="relative bg-white border border-border rounded-[18px] p-7 shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:border-primary hover:-translate-y-1 hover:shadow-float transition-all duration-300 flex flex-col"
                  style={{
                    opacity:   visible[`svc-${i}`] ? 1 : 0,
                    transform: visible[`svc-${i}`] ? 'none' : 'translateY(28px)',
                    transition: `opacity 0.5s ease ${i * 0.1}s, transform 0.5s ease ${i * 0.1}s, box-shadow 0.2s, border-color 0.2s`,
                  }}
                >
                  {/* Step number badge */}
                  <div className="absolute -top-3.5 left-7 bg-primary text-white text-[11px] font-bold px-3 py-1 rounded-full tracking-wide">
                    Langkah {s.step}
                  </div>

                  {/* Icon circle */}
                  <div className="w-[56px] h-[56px] rounded-[14px] bg-primary-lighter flex items-center justify-center mb-5 mt-2">
                    {s.icon}
                  </div>

                  <h3 className="font-body text-[18px] font-bold mb-2.5 text-ink tracking-[-0.3px]">{s.title}</h3>
                  <p className="text-[14px] text-muted leading-[1.75] flex-1">{s.desc}</p>

                  <div className="mt-5 inline-flex items-center gap-1.5 text-[12px] font-semibold text-primary bg-primary-light px-3 py-1.5 rounded-full self-start">
                    <Check size={11} strokeWidth={3} /> {s.tag}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-12 text-center">
            <button
              className={`${btnPrimary} text-[15px] py-3.5 px-10`}
      ...
      <section id="features" className="py-[100px] px-10 bg-surface">
        <div className="max-w-[1100px] mx-auto">
          ...
          <div className="grid gap-5" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))' }}>
            {features.map((f, i) => (
              ...
              >
                ...
                <h3 className="font-body text-[18px] font-bold mb-2.5 tracking-[-0.3px] text-ink">{f.title}</h3>
          ...
          <div className="mt-12 rounded-[20px] p-10 flex items-center justify-between flex-wrap gap-6" style={{ background: 'linear-gradient(135deg, #14532d, #16a34a)' }}>
            <div>
              <h3 className="font-body text-2xl font-bold text-white mb-2">Siap memulai perjalananmu?</h3>
      ...
      <section id="faq" className="py-[100px] px-10 bg-white">
        <div className="max-w-[760px] mx-auto">
          <div className="text-center mb-14">
            ...
            <h2 className="font-body font-bold mb-4 tracking-[-1px]" style={{ fontSize: 'clamp(28px, 4vw, 44px)' }}>
      ...
      <section className="py-[100px] px-10 text-center" style={{ background: 'linear-gradient(135deg, #14532d 0%, #0f1f12 100%)' }}>
        ...
        >
          ...
          <h2 className="font-body font-bold mb-4 tracking-[-1px] text-white" style={{ fontSize: 'clamp(28px, 5vw, 48px)' }}>
            Mulai sekarang, gratis.
          </h2>
          ...
          <div className="flex justify-between mb-12 flex-wrap gap-10 flex-col md:flex-row">
            <div className="max-w-[280px]">
              <div className="flex items-center gap-2.5 mb-4">
                ...
                <span className="font-body font-bold text-[17px] text-white">HabitTracker</span>
              </div>
            ...
            {[
              ...
              <div key={i}>
                <div className="font-body font-bold text-white text-sm mb-4 tracking-[0.3px]">{col.title}</div>
      ...
      {showCookieBanner && !showCookieModal && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-48px)] max-w-[900px] bg-white border border-border rounded-[16px] py-5 px-7 shadow-[0_8px_40px_rgba(0,0,0,0.12)] z-[300] animate-slide-up flex items-center gap-5 flex-wrap flex-col md:flex-row">
          <div className="flex items-start gap-3.5 flex-1 min-w-0">
            ...
            <div>
              <p className="font-body font-bold text-[15px] text-ink mb-1 m-0">Kami menggunakan cookie</p>
          ...
          <div className="bg-white rounded-[20px] max-w-[540px] w-full max-h-[90vh] overflow-y-auto shadow-[0_32px_80px_rgba(0,0,0,0.25)] animate-fade-in-scale">
            ...
            <div className="px-7 pt-7 pb-5 border-b border-border flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-body font-bold text-[17px] text-ink m-0">Pengaturan Cookie</h3>
            ...
            <div className="px-7 py-5 flex flex-col gap-3">
              ...
              {[
                { key: 'essential' as const,   icon: <Lock size={18} color="#4b7a54" />,     title: 'Cookie Esensial',   sub: 'Selalu aktif — wajib untuk fungsi dasar',              desc: 'Menyimpan token sesi login (jwt_token) agar Anda tetap masuk saat berpindah halaman. Tanpa cookie ini, Anda harus login ulang setiap saat. Tidak dapat dinonaktifkan.', disabled: true },
                { key: 'analytics' as const,   icon: <BarChart2 size={18} color="#4b7a54" />, title: 'Cookie Analitik',   sub: 'Statistik kunjungan disimpan di server kami',          desc: 'Merekam data kunjungan (perangkat, browser, OS, waktu akses) ke database internal kami. Data ini hanya dapat dilihat oleh administrator di panel Admin → Pengunjung untuk memantau statistik pengguna. Tidak ada pihak ketiga yang terlibat.' },
                { key: 'marketing' as const,   icon: <Megaphone size={18} color="#4b7a54" />, title: 'Cookie Komunitas',  sub: 'Mengizinkan berbagi ke wall komunitas',                desc: 'Diperlukan agar Anda dapat membuat postingan dan berbagi progress habit ke wall komunitas publik. Jika dinonaktifkan, fitur membuat postingan baru di halaman Komunitas tidak dapat digunakan.' },
                { key: 'preferences' as const, icon: <Settings size={18} color="#4b7a54" />,  title: 'Cookie Preferensi', sub: 'Mengaktifkan pengingat & notifikasi habit',            desc: 'Diperlukan untuk mengaktifkan fitur pengingat (reminder) otomatis pada setiap habit. Jika dinonaktifkan, Anda tidak dapat mengatur waktu pengingat harian untuk habit Anda.' },
              ].map(cat => (
                <CookieCategoryItem
                  key={cat.key}
                  icon={cat.icon}
